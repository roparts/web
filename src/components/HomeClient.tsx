...
export function HomeClient({ initialParts }: HomeClientProps) {
  ...
  const subcategories = useMemo(() => {
    const relevantParts = initialParts.filter(part => part.mainCategory === selectedMainCategory);
    const allSubcategories = relevantParts.map(part => part.subcategory);
    return ['All', ...Array.from(new Set(allSubcategories))];
  }, [selectedMainCategory, initialParts]);
